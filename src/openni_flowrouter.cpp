...
XnVSteadyDetector g_SteadyDetector;
...
void XN_CALLBACK_TYPE Broadcaster_OnActivate(void* cxt){
    printf("Broadcaster_OnActivate\n");
}
void XN_CALLBACK_TYPE Broadcaster_OnDeactivate(void* cxt){
    printf("Broadcaster_OnDeactivate\n");
}
void XN_CALLBACK_TYPE Push_Pushed(XnFloat fVelocity, XnFloat fAngle, void* cxt){
    printf("Pushed!\n");
    //printf("Push - Minimum Angle %f\n",g_PushDetector.GetPushMaximumAngleBetweenImmediateAndZ());
    //printf("Push - Maximum Angle %f\n",g_PushDetector.GetPushMinimumAngleBetweenImmediateAndPrevious());
	g_FlowRouter->SetActive(&g_SwipeDetector);
}
void XN_CALLBACK_TYPE Swipe_SwipeUp(XnFloat fVelocity, XnFloat fAngle, void* cxt){
    printf("Swipe_SwipeUp!\n");
    // printf("Swipe Motion Threshold%f\n",g_SwipeDetector.GetMotionSpeedThreshold());
    // printf("Swipe Steady Duration%f\n",g_SwipeDetector.GetSteadyDuration());
	g_FlowRouter->SetActive(&g_PushDetector);
}
void XN_CALLBACK_TYPE Swipe_SwipeDown(XnFloat fVelocity, XnFloat fAngle, void* cxt){
    printf("Swipe_SwipeDown!\n");  
	g_FlowRouter->SetActive(&g_PushDetector);
}
void XN_CALLBACK_TYPE Swipe_SwipeLeft(XnFloat fVelocity, XnFloat fAngle, void* cxt){
    printf("Swipe_SwipeLeft!\n");  
	g_FlowRouter->SetActive(&g_PushDetector);
}
void XN_CALLBACK_TYPE Swipe_SwipeRight(XnFloat fVelocity, XnFloat fAngle, void* cxt){
    printf("Swipe_SwipeRight!\n");
	g_FlowRouter->SetActive(&g_PushDetector);

}
void XN_CALLBACK_TYPE Steady_OnSteady(XnUInt32 nId, XnFloat fVelocity, void* cxt){
	printf("Steady\n");
	g_SteadyDetector.Reset();
}
int main(int argc, char **argv) {
    ros::init(argc, argv, "openni_tracker");
    ros::NodeHandle nh;

    string configFilename = ros::package::getPath("openni_test") + "/Sample-Tracking.xml";
 ...
 //    XnCallbackHandle hUserCallbacks;
	...
	g_FlowRouter = new XnVFlowRouter;
	...
	g_pSessionManager->AddListener(g_FlowRouter);
	g_FlowRouter->SetActive(&g_SteadyDetector);
    g_PushDetector.RegisterPush(&g_Context, &Push_Pushed);
    g_SwipeDetector.RegisterSwipeUp(&g_Context, &Swipe_SwipeUp);
    g_SwipeDetector.RegisterSwipeDown(&g_Context, &Swipe_SwipeDown);
    g_SwipeDetector.RegisterSwipeLeft(&g_Context, &Swipe_SwipeLeft);
    g_SwipeDetector.RegisterSwipeRight(&g_Context, &Swipe_SwipeRight);
	g_SteadyDetector.RegisterSteady(&g_Context, &Steady_OnSteady);
